...
public class FakeUserDataInitializer implements CommandLineRunner {
    ...
    public FakeUserDataInitializer(@Value("true") boolean enabled, UserRepository userRepository) {
    ...
    private void createUsers() {
        ...
        for (int i = 0; i < CHROME_USERS; i++) {
            users.add(User.builder()
                    ...
                    .enabled(true)
                    .roles(Set.of(new Role(ERole.CHROME_EXTENSION.getName()))).build()
            );
    ...
    private void createAdmin() {
        User user = User.builder()
                ...
                .enabled(true)
                .roles(Set.of(new Role(ERole.ROLE_ADMIN.getName()))).build();
